async function loadDesercion() {
    const content = document.getElementById('content');
    content.innerHTML = '';
    const filters = getFilters();
    const params = new URLSearchParams(filters);
    const res = await fetch(`http://localhost:5000/desercion?${params}`);
    const data = await res.json();

    data.forEach(item => {
        item['%REPR'] = isNaN(parseFloat(item['%REPR'])) ? 0 : parseFloat(item['%REPR']).toFixed(1);
    });
      ...
      <table class="min-w-full divide-y divide-gray-200 text-base">
        <thead>
          <tr>
            ...
            <th class="px-4 py-2 text-right text-gray-600 font-semibold">% Reprobados</th>
          </tr>
        </thead>
        <tbody>
          ${data.map(row => `
            <tr class="hover:bg-gray-100">
              ...
              <td class="border px-4 py-2 text-right">${row['%REPR']}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    const options = {
        series: [{
            name: '% Reprobados',
            data: data.map(item => parseFloat(item['%REPR'])),
        }],
        chart: {
            type: 'bar',
            height: 350,
