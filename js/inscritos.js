async function loadInscritos() {
    ...
    const res = await fetch(`http://localhost:5000/inscritos?${params}`);
    const data = await res.json();

    data.forEach(item => {
        item['_INS'] = isNaN(parseFloat(item['_INS'])) ? 0 : parseFloat(item['_INS']).toFixed(0);
    });
      ...
      <table class="min-w-full divide-y divide-gray-200 text-base">
        <thead>
          <tr>
            ...
            <th class="px-4 py-2 text-right text-gray-600 font-semibold">Inscritos</th>
          </tr>
        </thead>
        <tbody>
          ${data.map(row => `
            <tr class="hover:bg-gray-100">
              ...
              <td class="border px-4 py-2 text-right">${row['_INS']}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    const options = {
        series: [{
            name: 'Inscritos',
            data: data.map(item => parseFloat(item['_INS'])),
        }],
        chart: {
            type: 'bar',
            height: 350,
